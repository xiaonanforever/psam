...
public class ClusterMessageServiceFactory extends MessageServiceFactory {
	...
	public static ClusterMessageService startClusterService() throws ServiceClusterReadyException {
		LOGGER.info("准备Cluster Message Service ...");
		...
		for (int i = 0; i < psamServerList.size(); i++) {
			...
			LOGGER.info("处理第[{}]个服务连接-[{}]...", i, ss.getId());
			...
			MessageServiceImpl ms = (MessageServiceImpl) create(terminalNo, sc, mc, new MessageServiceImpl());
			...
			reqMsg.getBody().setTerminalPassword("123456");//TODO 登陆密码HardCode
